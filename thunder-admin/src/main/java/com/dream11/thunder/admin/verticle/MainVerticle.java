package com.dream11.thunder.admin.verticle;
...
public class MainVerticle extends AbstractVerticle {
    ...
    public Completable rxStart() {
        return ConfigUtil.getAdminConfig(vertx)
                .map(config -> {
                    this.config = config;
    ...
    private Completable deployRestVerticle(Config cfg) {
        ...
        int port = server.getPort() != null ? server.getPort() : 8081;
        ...
        log.info("Starting Thunder Admin application on {}:{} with {} instance(s)", host, port, instances);
