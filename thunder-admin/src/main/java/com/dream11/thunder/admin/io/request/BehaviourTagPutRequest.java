...
@Schema(description = "Request to update an existing Behaviour Tag. " +
...
public class BehaviourTagPutRequest {
  ...
  @Schema(
      description = "Updated exposure rules defining when this behaviour tag applies to users. " +
  ...
  @Schema(
      description = "Updated CTA relation rules defining which CTAs are shown/hidden for this behaviour tag. " +
                   "shownCta defines which CTAs to show (rule: LIST/ANY/NONE), " +
                   "hideCta defines which CTAs to hide (rule: LIST/ANY/NONE).",
      examples = {
          "{\"shownCta\": {\"rule\": \"ANY\"}, \"hideCta\": {\"rule\": \"NONE\"}}"
      }
  )
