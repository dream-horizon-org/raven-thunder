...
class AdminApiIT {
  ...
  void getCtas_returns200() {
    given().header("x-tenant-id", "default").when().get("/thunder/ctas/").then().statusCode(200);
  }

  @Test
  @DisplayName("GET /thunder/filters/ returns 200")
  void getFilters_returns200() {
    given().header("x-tenant-id", "default").when().get("/thunder/filters/").then().statusCode(200);
  }
