...
/**
 * Utility class for managing shared data across Vert.x instances.
 * Provides thread-safe singleton management with support for named instances.
 */
@Slf4j
public final class SharedDataUtils {
    ...
    public static <T> void put(Vertx vertx, T instance, String key) {
        ...
        LocalMap<String, ThreadSafe<T>> sharedDataMap = vertx.sharedData().getLocalMap(SHARED_DATA_MAP_NAME);
    ...
    /**
     * Thread-safe wrapper for shared objects.
     * Implements Shareable to ensure proper serialization in Vert.x.
     */
