...
/**
 * Test setup extension that:
 * - Starts an Aerospike container
 * - Configures RestAssured to talk to the Thunder API on a free port
 * - Starts the Thunder API Vert.x application
 * - Cleans up resources after all tests
 */
public class Setup implements ExtensionContext.Store.CloseableResource, BeforeAllCallback, TestInstancePostProcessor {
	...
	private void initializeAerospikeContainer() {
		this.aerospikeContainer = new GenericContainer<>(DockerImageName.parse("aerospike/aerospike-server:6.4.0.3"))
	...
	private void initializeGuiceInjector() {
		try {
			com.dream11.thunder.api.injection.GuiceInjector.initialize(
					java.util.List.of(new com.dream11.thunder.api.injection.MainModule(vertx.getDelegate())));
	...
	private void startApplication() {
		this.vertx.rxDeployVerticle("com.dream11.thunder.api.verticle.MainVerticle").blockingGet();
	}
